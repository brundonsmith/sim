...
// General utility for querying mouse, keyboard, and controller 
// input, in a loop-based style.
...
class Input {
  ...
  private mouseState: {[key: string]: Symbol} = {};
  ...
  public keyUp(key: string): boolean {
    return this.keyState[key.toLowerCase()] == null || this.keyState[key.toLowerCase()] === UP || this.keyState[key.toLowerCase()] === RELEASED;
  }
  ...
  public mouseButtonUp(button: number): boolean {
    return this.mouseState[button] == null || this.mouseState[button] === UP || this.mouseState[button] === RELEASED;
  }
